...
export default function Navbar() {
    ...
    const isNonMobilDevice = useMediaQuery("(min-width: 1000px)")

    useEffect(() => {
        const fetchUser = async () => {
            const res = await fetch("https://contactbackend-6qfi.onrender.com/api/user/auth", {
                method: "GET",
                headers: {
    ...
    return (
        <AppBar sx={{ p: "0 5%" }}>
            <Toolbar sx={{ justifyContent: "space-between" }}>
                <h4>Contact Handler</h4>
                <Box sx={{ display: "flex", alignItems: "center", gap: 3 }}>
                    {isNonMobilDevice ? <>
                        {auth ? <><Button><Link style={{ color: "#fff", textDecoration: "none" }} to={"/"}>Home</Link></Button>
